...
def fromBidInfoMessage(bid: BidInfo):
    return Agent.BidInformation(y=bid.winning_score, z=bid.winning_agent, t=bid.timestamp, j=bid.task_id, k=bid.sender_id)
...
def toBidInfoMessage(bid: Agent.BidInformation):
    return BidInfo(winning_agent=bid.z, task_id=bid.j, timestamp=float(bid.t), sender_id=bid.k)


def toBidInfoListMessage(bids: List[Agent.BidInformation], agent_id) -> BidInfoList:
    bid_list = []
    for bid in bids:
